...
local function only_no_name_modified()
	local modified_buffers = vim.tbl_filter(function(bufnr)
		return vim.bo[bufnr].modified
	end, vim.api.nvim_list_bufs())

	return #modified_buffers == 1 and vim.api.nvim_buf_get_name(modified_buffers[1]) == ''
end

-- Function to handle quitting when only [No Name] is modified
local function quit_ignore_no_name()
	if only_no_name_modified() then
		vim.cmd('qa!')
	else
		vim.cmd('confirm qa')
	end
...
-- Allow closing neovim, if the [No Name] buffer is not saved.
vim.api.nvim_create_autocmd({ "BufWritePre", "FileWritePre" }, {
	callback = function()
		vim.bo.modified = not only_no_name_modified()
	end
})

-- Create a user command for quitting
vim.api.nvim_create_user_command('QA', quit_ignore_no_name, {})

-- Override the built-in :qa command
